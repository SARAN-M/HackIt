...
module.exports.paginatedResults = async (req, res, next) => {
    ...
    if (endIndex < collection.find().count()) {
        results.next = {
            page: page + 1,
            limit: limit
        }
    ...
    try {
        collection.find().limit(limit).skip(startIndex).toArray(function(err, docs){
            if(err){
            ...
            collectionChunks.find({files_id : docs[0]._id}).sort({n: 1}).toArray(function(err, chunks){
                ...
                res.render('imageView.pug', {title: 'Image File', message: 'Image loaded from MongoDB GridFS', imgurl: finalFile});
            });
            } 
        });
    } catch (e) {
  ...
  MongoClient.connect(url, {useUnifiedTopology: true}, function(err, client){
    ...
    collection.find().toArray((err, files) => {
      ...
      if (!files || files.length === 0) {
        ...
        res.render('test.html', { files: files });
      }
    });
  });
};